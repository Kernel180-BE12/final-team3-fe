...
export default function SignupPage() {
  ...
  const handleRequestVerification = async () => {
    ...
    try {
      // 실제 API 연동
      const response = await fetch('http://localhost:8580/auth/email/otp/request', {
        method: 'POST',
  ...
  const handleConfirmVerification = async () => {
    ...
    try {
      // 실제 API 연동
      const response = await fetch('http://localhost:8580/auth/email/otp/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const isFormValid = formData.email && isVerified && formData.password && formData.passwordConfirm && formData.nickname && formData.termsAgreed && formData.privacyAgreed;
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mt-1">
              <input id="email" name="email" type="email" required disabled={isVerified} value={formData.email} onChange={handleChange} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed" />
              <button
                type="button"
              ...
              >
                {isVerified ? '인증 완료' : (isVerificationSent ? '재전송' : '인증 요청')}
              </button>
            </div>
          </div>
          ...
          {isVerificationSent && !isVerified && (
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                <button type="button" onClick={handleConfirmVerification} className="flex-shrink-0 px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  확인
                </button>
              </div>
              {/* 타이머 만료시 재전송 안내 */}
              {canResend && (
                <div className="mt-1 text-sm text-blue-500">인증코드 유효시간이 만료되었습니다. "재전송"을 눌러주세요.</div>
              )}
            </div>
          )}
